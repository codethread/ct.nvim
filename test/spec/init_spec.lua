...
describe("Spec", function()
	describe("namespaces", function()
		...
		it("should create a namespace and retrieve it", function()
			...
			assert.is_true(s:is_valid(function() return true end, nil))
	...
	describe("namespace opts", function()
		it("should allow duplicate if set", function()
			local s = setup { on_key_clash = "ignore" }
			s:def("int dupe", s:spec(is_the_anwser))
			s:def("int dupe", s:spec(is_the_anwser))
		end)
	end)
	...
	describe("s:explain", function()
		it("should print out error", function()
			local s = setup()
			assert.are_equal(s:explain(s:or_ { "int" }, 10, { str = true }), "Success\n")
		end)
	end)
	...
	describe("s:def", function()
		it("should error on duplicate", function()
			local s = setup()
			s:def("int dupe", s:spec(is_the_anwser))
			assert.has_error(function() s:def("int dupe", s:spec(is_the_anwser)) end)
		end)

		it("should create a spec", function()
			local s = setup()
			s:def("int dupe", s:spec(is_the_anwser))
			assert.is_true(s:is_valid("int dupe", 42))
			assert.is_false(s:is_valid("int dupe", 3.4))
		end)
	end)

	describe("s:and_", function()
