...
local function fmt(text, color, isBold)
	local colorSequence = getColorStyleSequence(color, isBold or false)
		...
		if not cliArgs.skip_tests or not fname:match(".+_spec.lua$") then
			for _, e in ipairs(errors) do
				if not vim.list_contains(filters, e.message) then
					...
					io.write(
						fmt(fname, Color.CYAN)
							.. ":"
							.. fmt(tonumber(e.range.start.line) + 1, Color.YELLOW)
							.. ":"
							.. fmt(tonumber(e.range.start.character) + 1, Color.YELLOW)
							.. " - "
							.. fmt(e.severity, Color.RED)
							.. " "
							.. fmt(e.code .. ":", Color.GREY)
							.. "\n"
							.. e.message
					)
					io.write("\n\n")
